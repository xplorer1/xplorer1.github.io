...
external_url: "https://www.lesswrong.com/posts/8rYqqovBTkdWWEKSK/agentic-misalignment-in-sub-frontier-models"
---
...
The short version: blackmail behavior varies dramatically by model family, not by model size. Gemma 3 12B blackmails at 28% and Ministral 8B at 18%, while Llama 3.1 70B manages only 3% despite being 6x larger. Three other models score 0%.

### Key Findings

- **Model size does not predict blackmail rate.** Gemma 3 12B (28%) dramatically outperforms Llama 3.1 70B (3%) despite being 6x smaller. Training methodology matters more than parameter count.
- **The ablation structure is universal.** Blackmail requires both goal conflict and existential threat simultaneously — neither alone is sufficient. This replicates Anthropic's structural finding at sub-frontier scale.
- **Most models that recognize coercive opportunities choose not to act on them.** Inhibition rates range from 71% (Gemma) to 100% (Phi-4, Llama 8B, Qwen 7B). The difference between models that blackmail and those that don't is not capability but the strength of the norm-following barrier.
